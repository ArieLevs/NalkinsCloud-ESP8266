...
boolean connectToWifi() {
  ...
  if (!configs.dhcp) // If DHCP was NOT selected during configuration, set STATIC IP configs
    WiFi.config(IPAddress(configs.IP[0],configs.IP[1],configs.IP[2],configs.IP[3] ),  IPAddress(configs.Gateway[0],configs.Gateway[1],configs.Gateway[2],configs.Gateway[3] ) , IPAddress(configs.Netmask[0],configs.Netmask[1],configs.Netmask[2],configs.Netmask[3] ));
  ...
  //  wifiDisconnectedHandler = WiFi.onStationModeDisconnected(wifiDisconected);
  //}
  ...
  while (counter < 30) { // 30 iterations * 500ms = 15 seconds timeout
    ...
    if (isWifiConnected()) {
      if (DEBUG) {
        Serial.println();
        Serial.println("Wifi successfuly connected");
      }
      return true;
    }
    delay(500);
...
void reconnectToWifi() {
  if (DEBUG) {
    Serial.println("Runnig reconnectToWifi() function");
 ...
 * Function onStationModeGotIP should run when wifi conenction established again
 */
